...
# staff mail ids 
staff_mails = ['E132935@esoft.academy', 'E132935@esoft.academy'] 
...
def check(no_of_days, row_num, b): 
	...
	for student in range(0, len(row_num)): 
		...
		if l2 != "" and len(l3) != 0: 
			...
			if 1 <= b <= len(staff_mails):
				staff_id = staff_mails[b-1] # pick respective staff's mail_id 
				mailstaff(staff_id, msg2) # mail to staff
			else:
				print(f"Invalid subject code: {b}. No staff email found.")

# for students 
def mailstu(li, msg): 
	from_id = 'tproject310@gmail.com'
	pwd = 'xtxgnykqfvwjiins '
